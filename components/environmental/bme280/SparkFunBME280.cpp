...
BME280::BME280( uint8_t address, TwoWire *UseWire )
{
...
void BME280::readRegisterRegion(uint8_t *outputPointer , uint8_t offset, uint8_t length)
{
	...
	_Wire->beginTransmission(settings.I2CAddress);
	_Wire->write(offset);
	_Wire->endTransmission();

	// request bytes from slave device
	_Wire->requestFrom(settings.I2CAddress, length);
...
uint8_t BME280::readRegister(uint8_t offset)
{
	...
	{
		result = _Wire->read(); // receive a byte as a proper uint8_t
	}
